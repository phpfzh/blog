...
public class UserServiceImpl extends ServiceImpl<UserMapper,User> implements UserService {
	...
	public  ProcessBack userRegisterByPC(HttpServletRequest request,UserRegVo userRegVo) {
		...
		try{
			...
			if(userRegVo.getEmail() != null && StringUtil.isNotEmpty(userRegVo.getEmail())){
  			...
  			//手机短信验证码
			String ssmCode = (String)request.getSession().getAttribute(Session_Constat.USER_SSM_CODE);;
			//发送手机短信验证码手机号
			String ssmMobile = (String) request.getSession().getAttribute(Session_Constat.USER_MOBILE);;
			if(StringUtil.isEmpty(ssmCode) || StringUtil.isEmpty(ssmMobile)){
 				processBack.setCode(ProcessBack.FAIL_CODE);
				processBack.setMessage("短信验证码错误或已过期");
				return processBack;
			...
			members.setIsadmin(0);
 			...
 			//缓存用户基本信息，安全信息
 			request.getSession().setAttribute(Session_Constat.USER, commonMember);
 			request.getSession().setAttribute(Session_Constat.USERCENTER, members);
 			
 			//删除缓存的手机验证码，手机号
 			request.getSession().removeAttribute(Session_Constat.USER_SSM_CODE);
			request.getSession().removeAttribute(Session_Constat.USER_MOBILE);
	...
	* @Description: TODO(注册时发送手机短信验证码[PC]) 
	* @param @param request
	* @param @return    设定文件 
	* @return AjaxResult    返回类型 
	* @throws
