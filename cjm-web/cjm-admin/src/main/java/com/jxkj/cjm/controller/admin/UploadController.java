...
public class UploadController extends BaseController {
    ...
    public Map<String, Object> uploadCoverimg(@RequestParam("upfile") MultipartFile multipartFile) {
        Map<String, Object> res = new HashMap<>();
        try {
            //图片上传方法
            ProcessBack processBack  = forumThreadCoverimgService.uploadCoverimg(multipartFile);
            System.out.println(JSON.toJSONString(processBack));
            if (processBack.getCode().equals(ProcessBack.SUCCESS_CODE)) {
                res.put("url", processBack.getData());//上传的图片路径
                res.put("state", "SUCCESS");
                res.put("title", multipartFile.getOriginalFilename());
