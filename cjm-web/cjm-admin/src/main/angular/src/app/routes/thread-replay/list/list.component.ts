...
export class ThreadReplayListComponent implements OnInit {
  basePath: string;
  url: string;
  ...
  ngOnInit() {
    this.url = `${this.basePath}/api/forumThreadReplay/list`;
  }
  ...
  auditBatch(status: number) {
    ...
    this.threadReplayService.audit(status, tids)
      .subscribe(rep => {
        if (rep.code == "88") {
